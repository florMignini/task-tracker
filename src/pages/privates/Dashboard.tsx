          ...
          <div className="w-[95%] md:w-[99%] flex flex-wrap items-center justify-start gap-2 ">
            {projects &&
              projects.map((project: IProject) => (
                <div
                  key={project?._id}
                  className="w-[35%] xl:w-1/4 h-[160px] bg-[#5E1914]/60 px-2 flex flex-col rounded-lg items-center shadow-2xl hover:scale-[1.01] transition-transform text-[#5E1914] border-[1px]"
                >
                  <div className="w-[90%] h-[90%] flex items-center justify-start flex-col">
                    ...
                    <div className="w-[90%] flex flex-col items-center justify-start gap-2 py-2">
                     ...
                     <div className="w-fit p-1 h-[20px] rounded-md bg-emerald-600 flex items-center justify-center font-semibold text-zinc-700 text-xs gap-1">
                        <p className="flex">
                          {differenceInBusinessDays(
                            new Date(
                              Number(
                                project?.deadline!.split("T")[0].split("-")[0]
                              ),
                              Number(
                                project?.deadline!.split("T")[0].split("-")[1]
                              ),
                              Number(
                                project?.deadline!.split("T")[0].split("-")[2]
                              )
                            ),
                            new Date(
                              Number(date.toJSON().split("T")[0].split("-")[0]),
                              Number(date.toJSON().split("T")[0].split("-")[1]),
                              Number(date.toJSON().split("T")[0].split("-")[2])
                            )
                          )}
                        </p>
                      ...
                      <div className="w-fit p-1 h-[20px] rounded-md bg-[#E1BDB5]/40 flex items-center justify-center font-semibold text-[#5E1914] text-xs gap-1">
                       {
                        project && project?.tasks &&(
                          <p>{(project?.tasks?.length < 2) ? `${project?.tasks?.length} task` : `${project?.tasks?.length} tasks`}  </p>
                        )
                       }
                      </div>
                     </div>
